...
export default function HeroSection() {
  ...
  if (loading) {
    return (
      <div className="bg-[#fdfdfc] border border-[#f1f0ef] rounded-2xl h-48 w-full max-w-[640px] mx-auto mt-16 flex items-center justify-center">
  ...
  return (
    <div className="backdrop-blur-md bg-white/50 border border-white rounded-2xl h-64 w-full max-w-[640px] mx-auto relative overflow-hidden shadow-md">
      ...
      {/* Random poem badge */}
      <div className="absolute top-[-4px] right-4 bg-stone-200 px-2.5 py-2.5 rounded-lg">
        <span className="text-xs text-stone-600">شعر تصادفی</span>
      </div>

      {/* Poem content */}
      <div className="relative top-1/2 transform -translate-y-1/2 flex items-center gap-6 justify-center">
        <p className="text-lg font-bold text-stone-900 text-right tracking-tight font-doran">
          {randomPoem.verses[0] || 'صبا به لُطف بگو آن غزالِ رَعنا را'}
        </p>
        
        {/* Decorative line */}
        <div className="w-6 h-px bg-stone-600"></div>
      ...
      <div className="absolute bottom-4 left-4 flex items-center gap-1 flex-row-reverse">
        ...
        <span className="text-xs text-stone-600 font-normal tracking-tight">
