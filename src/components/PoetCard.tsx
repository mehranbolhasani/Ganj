...
export default function PoetCard({ poet }: PoetCardProps) {
  ...
  return (
    <Link 
      href={`/poet/${poet.id}`}
      className="block backdrop-blur-md bg-white/50 dark:bg-gray-800/50 border border-white rounded-xl shadow-lg/5 hover:shadow-sm transition-all duration-200 w-full flex-1 min-w-[310px]"
    >
      <div className="flex items-center justify-between p-4 flex-row-reverse">
        ...
        <div className="flex flex-col gap-4 items-start w-full">
          <h3 className="font-doran text-md font-bold text-stone-900 dark:text-white text-right leading-4">
            {poet.name}
          </h3>
          
          {yearRange && (
            <div className="flex items-center gap-1 w-full justify-end flex-row-reverse">
              <span className="text-sm text-stone-500 dark:text-gray-400 text-right leading-4">
                {yearRange}
              </span>
